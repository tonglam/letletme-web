...
`$gh-codex-review-loop` for PR work: every P0-P3 finding must be dispositioned
and its thread resolved, with only tests/scripts P2/P3 receiving the
disposition-only time exception. A quota override skips only a new review
request, never findings, CI, or cleanup. After merge, clean only the exact
matching worktree, local branch, and remote branch.
