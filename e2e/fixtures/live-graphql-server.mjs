...
const requiresLivePointsV2Contract = query =>
	/\b(?:calcLivePointsByEntry|calcLivePointsForEntries|liveScores|playerLive|eventLive|eventLiveExplain|eventLiveExplains|liveSnapshot|liveContext|entryLiveCompetitionBoard|leagueLiveHead|tournamentOfficialH2H|tournamentOfficialH2HHistory|tournamentSelectionIndex|tournamentEntrySquads|tournamentDetailDesk|gameweekDesk|homeGameweek)\s*(?:\(|\{)/.test(
		query
	)
...
const server = createServer((request, response) => {
	...
	request.on('end', async () => {
		...
		if (rule?.error) {
			json(response, 200, { errors: [{ message: 'Injected fixture failure', ...(rule.errorCode ? { extensions: { code: rule.errorCode } } : {}) }] })
			return
		}
