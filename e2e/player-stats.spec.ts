...
test('two-player deep link is server-seeded with zero browser desk requests', async ({
	...
	expect(
		routeReady.every(sample => sample.measurementKind === 'initial_navigation')
	).toBe(true)
	expect(
		await page.evaluate(
...
test.describe('SSR detail stream', () => {
	test.skip(
		...
		'Run the fixture-control suite separately with one worker'
	)
	...
	test('directory is interactive before a slow initial overview, and a new choice wins', async ({
		page
	}, testInfo) => {
		const initialPlayerId = runPlayerId(1)
		await control([
			{
				operation: 'GetPlayerStatsDeskOverview',
				variables: { playerIds: [initialPlayerId] },
				delayMs: 3500
			}
		])
		const browserRequests: string[] = []
		page.on('request', request => {
			if (request.url().includes('/api/player-stats/desk?'))
		...
		const initial = (await (await fetch(fixture)).json()).requests.find(
			(row: { operation: string; variables: { playerIds?: number[] } }) =>
				row.operation === 'GetPlayerStatsDeskOverview' &&
				row.variables.playerIds?.[0] === initialPlayerId
		)
		expect(initial?.finishedAt).toBeNull()
		await players.getByRole('button', { name: /^Palmer/ }).click()
		await expect(
			page.getByRole('region', { name: 'Player overall' })
		).toContainText('Palmer')
		await expect(page).toHaveURL(/p1=2/)
		await expect
			.poll(
				async () =>
					(await (await fetch(fixture)).json()).requests.find(
						(row: { variables: { playerIds?: number[] } }) =>
							row.variables.playerIds?.[0] === initialPlayerId
					)?.finishedAt
			)
			.toBeTruthy()
		await expect(
			page.getByRole('region', { name: 'Player overall' })
		).not.toContainText('Saka')
		await expect(page).toHaveURL(/p1=2/)
		expect(
			browserRequests.filter(
				url =>
					new URL(url).searchParams.get('playerIds') === String(initialPlayerId)
			)
		).toHaveLength(0)
		await testInfo.attach('detail-stream-requests', {
			body: JSON.stringify(await (await fetch(fixture)).json(), null, 2),
			contentType: 'application/json'
		})
	})

	test('serves real overview HTML in a later response chunk without browser JavaScript', async ({
		browser,
		baseURL
	}, testInfo) => {
		...
		const response = await fetch(
			`${baseURL}/explore/player-stats?p1=${initialPlayerId}`
		)
		expect(response.status).toBe(200)
		const reader = response.body!.getReader()
		const decoder = new TextDecoder()
		let html = '',
			directoryMs: number | null = null,
			detailMs: number | null = null,
			fragmentAnchorMs: number | null = null
		...
		for (;;) {
			...
			if (directoryMs == null && html.includes('aria-label="Players"'))
				directoryMs = ms
			if (
				detailMs == null &&
				html.includes('data-player-stats-ssr-detail="true"')
			)
				detailMs = ms
			if (
				fragmentAnchorMs == null &&
				html.includes('data-player-stats-noscript-section="history"')
			)
				fragmentAnchorMs = ms
		}
		...
		try {
			...
			const noJsDetail = noJsPage.locator(
				'[data-player-stats-noscript-result="true"] [aria-label="Player overall"]'
			)
			...
			await noJsPage.goto(
				`${baseURL}/explore/player-stats?p1=${initialPlayerId}#ps-history`
			)
			const noJsFragmentAnchor = noJsPage.locator(
			...
			const fragmentPosition = await noJsPage.evaluate(() => ({
				top:
					document
						.querySelector('[data-player-stats-noscript-section="history"]')
		...
		await testInfo.attach('ssr-stream-timing', {
			body: JSON.stringify({ directoryMs, detailMs, chunks }, null, 2),
			contentType: 'application/json'
		})
	})

	test('a failed deep link settles once and offers an explicit retry', async ({
		page
	}) => {
		...
		page.on('request', request => {
			if (request.url().includes('/api/player-stats/desk?')) browserRequests++
		})
		await page.goto(`/explore/player-stats?p1=${initialPlayerId}`)
	...
	test('shows a truthful no-script terminal for a failed deep link', async ({
		browser,
		baseURL
	}) => {
		await control([
		...
		try {
			await noJsPage.goto(`${baseURL}/explore/player-stats?p1=1`)
			const terminal = noJsPage.locator(
				'[data-player-stats-noscript-result="true"]'
			)
	...
	test('clearing a comparison before its seed arrives keeps p2 cleared', async ({
		page
	}) => {
		...
		await page.goto(`/explore/player-stats?p1=2&p2=${initialPlayerId}`, {
			waitUntil: 'commit'
		})
