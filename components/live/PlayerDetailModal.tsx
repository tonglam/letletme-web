...
import { Award, Clock, ShieldCheck, Goal, Shirt, Zap, AlertCircle } from "lucide-react";
          ...
          <div className="flex justify-between items-center mb-3">
            <div className="flex items-center gap-2">
              <div className="relative w-8 h-8">
                <Image
                  src={`/team-logos/${player.teamShort.toLowerCase()}.png`}
                  alt={player.team}
                  fill
        ...
        <div className="p-5">
          ...
          <div className="grid grid-cols-2 gap-4 mb-5">
            <div className="bg-accent/30 rounded-lg p-3 flex flex-col items-center justify-center">
              <span className="text-muted-foreground text-xs mb-1">Ownership</span>
              <span className="text-lg font-semibold">{player.ownershipPercentage}%</span>
            </div>
            
            {player.bps > 0 ? (
              <div className="bg-accent/30 rounded-lg p-3 flex flex-col items-center justify-center">
                <span className="text-muted-foreground text-xs mb-1">BPS</span>
                <span className={`text-lg font-semibold ${getBpsColor(player.bps)}`}>
                  {player.bps}
                </span>
              </div>
            ) : (
          ...
          {player.bonusPoints > 0 && (
            <div className="bg-yellow-500/10 rounded-lg p-3 mb-5 flex items-center">
              ...
              <div>
                ...
                <p className="text-lg font-bold text-yellow-500">+{player.bonusPoints}</p>
              </div>
            </div>
          )}
          ...
          <div className="space-y-2 rounded-lg bg-accent/20 p-3">
            ...
            {player.stats.goals > 0 && (
              <div className="flex justify-between items-center py-2 border-b border-border/50 last:border-0">
                <div className="flex items-center">
                  ...
                  <span>
                    Goal{player.stats.goals > 1 ? 's' : ''} ({player.stats.goals})
                  </span>
                </div>
                <span className="font-semibold text-primary">
            ...
            {player.stats.assists > 0 && (
              <div className="flex justify-between items-center py-2 border-b border-border/50 last:border-0">
                <div className="flex items-center">
                  ...
                  <span>
                    Assist{player.stats.assists > 1 ? 's' : ''} ({player.stats.assists})
                  </span>
                </div>
                <span className="font-semibold text-primary">
            ...
            {player.stats.saves && player.stats.saves > 0 && (
              <div className="flex justify-between items-center py-2 border-b border-border/50 last:border-0">
                <div className="flex items-center">
                  <span className="w-4 h-4 flex items-center justify-center mr-2 text-muted-foreground">🧤</span>
            ...
            {player.stats.yellowCards > 0 && (
              <div className="flex justify-between items-center py-2 border-b border-border/50 last:border-0">
                <div className="flex items-center">
                  ...
                  <span>Yellow Card{player.stats.yellowCards > 1 ? 's' : ''}</span>
                </div>
          ...
          <div className="mt-5 pt-3 border-t border-border flex justify-between items-center">
            ...
            <span className="text-xl font-bold text-primary">{player.points}</span>
          </div>
        </div>
      </DialogContent>
    </Dialog>
