...
import { Activity, Award, BarChart2, Clock, Eye, User } from 'lucide-react'
...
export function MatchCard({ match }: MatchCardProps) {
	...
	const openPlayerDetail = (
		...
		const position =
			player.player.includes('Raya') || player.player.includes('Pope')
				? 'GKP'
				: player.player.includes('Gabriel')
				? 'DEF'
				: 'MID'

		const hasCleanSheet =
			player.player.includes('Raya') || player.player.includes('Gabriel')
		if (hasCleanSheet) {
			const csPoints =
									...
									>
										...
										<div className="text-muted-foreground flex flex-wrap gap-2 mt-2">
											...
											{player.bps !== undefined && (
												<span
													className={`bg-background rounded-full px-2 py-0.5 text-xs ${getBpsColor(
														player.bps
													)}`}
												>
													BPS: {player.bps}
