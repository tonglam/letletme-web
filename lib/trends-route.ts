...
export const TRENDS_PUBLIC_CACHE_CONTROL =
...
const jsonError = (
	message: string,
	status: number,
	cache = TRENDS_PRIVATE_CACHE_CONTROL
) =>
	NextResponse.json(
		{ error: message },
...
function upstreamStatus(error: unknown) {
	...
	const candidate = error as {
		status?: unknown
		extensions?: { http?: { status?: unknown } }
	}
...
export async function publicDeskRoute(request: Request) {
	...
	try {
		const data = {
			trendCohortSnapshot: await loadTrendDesk(
				parsed.cohortId,
				parsed.eventId,
				'PUBLIC',
				null,
		...
		console.info('[trends] public desk ready', {
		...
		const response = NextResponse.json(data, {
			headers: {
				'Cache-Control': TRENDS_PUBLIC_CACHE_CONTROL,
				ETag: etag(data)
			}
		})
		...
		console.error('[trends] public desk failed', {
			cohortId: parsed.cohortId,
			eventId: parsed.eventId,
			durationMs: Number((performance.now() - startedAt).toFixed(2)),
		...
		return jsonError(
			status === 404
				? 'Trends cohort not found'
				: 'Trends desk is temporarily unavailable',
			status
		)
	}
}
...
export async function myDeskRoute(request: Request) {
	...
	try {
		...
		console.info('[trends] private desk ready', {
			cohortId: parsed.cohortId,
			eventId: parsed.eventId,
			durationMs: Number((performance.now() - startedAt).toFixed(2))
		})
		return NextResponse.json(data, {
			headers: { 'Cache-Control': TRENDS_PRIVATE_CACHE_CONTROL }
		})
	} catch (error) {
		const status = upstreamStatus(error)
		return jsonError(
...
export async function myCohortsRoute() {
	...
	if (!session || !entryId) return jsonError('Authentication required', 401)
	try {
		return NextResponse.json(await loadTrendCohorts('MINE', session), {
			headers: { 'Cache-Control': TRENDS_PRIVATE_CACHE_CONTROL }
		})
	} catch {
