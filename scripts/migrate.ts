	...
	try {
		...
		try {
			...
			for (const role of ['anon', 'authenticated']) {
				...
				if (exists)
					await client.unsafe(
						`REVOKE ALL ON TABLE bauth.__drizzle_migrations FROM ${role}`
					)
			}

			const local = await loadLocalMigrations()
			const dedicatedRows = await client<
				{ hash: string; created_at: string }[]
			>`
				SELECT hash, created_at::text FROM bauth.__drizzle_migrations ORDER BY created_at
			`
			const dedicatedTimes = new Set(
				dedicatedRows.map(row => Number(row.created_at))
			)
			const [{ shared_exists: sharedExists }] = await client<
			...
			if (sharedExists) {
				...
				for (const migration of local.migrations) {
					...
					const matches = sharedRows.filter(
						row => Number(row.created_at) === migration.when
					)
					if (matches.length > 1) {
						throw new Error(
							`Shared migration ledger has duplicate timestamp ${migration.when}`
						)
					}
					const sharedRow = matches[0]
					if (!sharedRow) continue
					if (sharedRow.hash !== migration.hash) {
						throw new Error(
							`Shared migration checksum mismatch for ${migration.tag}`
						)
					}
					await client`
						INSERT INTO bauth.__drizzle_migrations (hash, created_at)
						VALUES (${sharedRow.hash}, ${migration.when})
					`
					dedicatedTimes.add(migration.when)
					console.log(`Adopted shared Web migration ${migration.tag}`)
				}
			}
			...
			assertMigrationHistory(afterAudit, true)
		} finally {
