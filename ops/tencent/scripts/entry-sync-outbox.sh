...
# release scripts. Only pass the cron bearer to curl; do not inherit the rest
# of the production environment into the request process.
CRON_SECRET=''
...
exec env -i PATH=/usr/bin:/bin /usr/bin/curl \
	...
	http://127.0.0.1:3000/api/cron/entry-sync-outbox
