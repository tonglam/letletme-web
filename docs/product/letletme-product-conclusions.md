...
Live Points, Live Tournaments, and Live Matches were built to provide fast, reliable results when official FPL did not provide a sufficient live experience. This remains the primary matchday job even if the source of the underlying facts changes.
...
| Tournaments | Prepared official-league tracking machinery with roster synchronization and weekly collection, plus custom membership, gameweek windows, points races, single/double knockouts, live standings, completed results, and selection metrics; additional race models exist in the backend | Present publicly as **Competitions** with tracked official leagues and custom tournaments. Share entry/league evidence across every tournament sourced from the same league while preserving each tournament's own result and audit. |
