...
test(
	'web security constraints serialize identity, session, and rate ownership',
	{
		skip: !enabled
	},
	async () => {
		...
		try {
			const ownership = await Promise.allSettled(
				userIds.map(
					(id, index) => sql`
		...
		`
			...
			assert.equal(
				ownership.filter(result => result.status === 'fulfilled').length,
				1
			)
		...
		`
			...
			assert.equal(
				sessions.filter(result => result.status === 'fulfilled').length,
				1
			)
