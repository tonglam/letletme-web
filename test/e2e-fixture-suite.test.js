...
test('SSR suite preserves selectors, serial execution and fixture environment', () => {
	...
	assert.equal(result.calls[4].marketHistory, '1')
	assert.equal(result.calls[4].existingBuild, '1')
	assert.ok(result.calls[4].args.includes('e2e/market-historical-freshness.spec.ts'))
})
