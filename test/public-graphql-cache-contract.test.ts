...
describe('public GraphQL cache contract', () => {
	...
	it('keeps personal Player Stats work behind the public bootstrap', async () => {
		...
		assert.match(globals, /\[hidden\]:has\(\[data-ssr-stream-content\]\)/)
		assert.match(globals, /body:has\(\[data-ssr-stream-content="player-stats-detail"\]\)[\s\S]*data-ssr-stream-fallback="player-stats-detail"/)
	...
	it('streams personal squad content into initial HTML before hydration', async () => {
		...
		assert.match(fixturesClient, /<Suspense fallback=\{<div data-ssr-stream-fallback="fixtures-squad"[\s\S]*<FixturesSquadStream[\s\S]*promise=\{squadPromise\}/)
		...
		assert.match(priceChangesBoard, /<Suspense fallback=\{<div data-ssr-stream-fallback="price-changes-squad"[\s\S]*<PriceChangesSquadStream[\s\S]*promise=\{personalSeedPromise\}/)
