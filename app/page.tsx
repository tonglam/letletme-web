...
function HomeHero() {
	const capabilities = [
		{ icon: Radio, label: 'Live points' },
		{ icon: BarChart3, label: 'Player analysis' },
		{ icon: Trophy, label: 'Private tournaments' },
	] as const
			...
			<div className="mx-auto grid w-full max-w-6xl gap-10 px-4 py-16 lg:grid-cols-[1.35fr_0.65fr] lg:items-end lg:px-8 lg:py-24">
				<div className="max-w-3xl">
					...
					<h1 className="max-w-3xl text-balance text-4xl font-bold leading-[1.02] tracking-[-0.035em] sm:text-6xl lg:text-7xl">
						See the gameweek before the rank settles.
					</h1>
					<p className="mt-6 max-w-2xl text-pretty text-lg leading-8 text-muted-foreground sm:text-xl">
						Live scoring, player form, and tournament context in one focused matchday workspace.
					</p>
					<div className="mt-8 flex flex-col gap-3 sm:flex-row">
						...
						<Button size="lg" variant="outline" asChild>
							<Link href="/data/player-stats">Compare players</Link>
						</Button>
					</div>
				</div>
...
function DeadlineSectionFallback() {
	return (
		<section className="border-b bg-primary/5">
			<div className="mx-auto flex max-w-4xl flex-col items-center gap-5 px-4 py-12" aria-label="Loading gameweek deadline" aria-busy="true">
			...
			<section className="bg-background py-8">
				<div className="mx-auto max-w-4xl px-4">
					<Card className="flex flex-col items-center gap-3 p-8 text-center">
						...
						<h2 className="text-2xl font-bold">Matchday insights unavailable</h2>
						<p className="max-w-lg text-muted-foreground">
							Team, transfer, and fixture insights will return when the live data service responds.
						</p>
					</Card>
				</div>
			</section>
...
function PageInsightsFallback() {
	return (
		<div className="mx-auto grid w-full max-w-4xl gap-8 px-4 py-8 md:grid-cols-2" aria-label="Loading dashboard insights" aria-busy="true">
