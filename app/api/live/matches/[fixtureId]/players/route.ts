...
async function handleGet(
	...
	try {
		const data = await executePublicServerQuery(
			'gameweek',
			GET_LIVE_FIXTURE_PLAYERS,
			{ ref: { season, eventId, scoreCoreRevision }, fixtureId },
			{ cache: 'no-store' }
		)
		return NextResponse.json(data, {
			headers: {
				'Cache-Control':
					'public, max-age=300, s-maxage=21600, stale-while-revalidate=86400, no-transform'
			}
		})
	} catch (error) {
		const status = String(error).includes('LIVE_SCORE_REVISION_GONE')
			? 409
			: 503
		return NextResponse.json(
			{
...
export function GET(
	...
	return withPublicRouteGraphQLIngress(request, () =>
