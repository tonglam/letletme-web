...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
	...
	const eventId = Number(params.get('eventId')); const revision = params.get('revision')
	...
	try {
		const data = await executeServerQuery(GET_TOURNAMENT_SELECTION_INDEX, { entryId: session.user.fplEntryId, tournamentId, ref: { season: String(getCurrentSeasonKey()), eventId, revision } }, { cache: 'no-store' })
		return NextResponse.json(data, { headers: { 'Cache-Control': 'private, no-store' } })
	} catch { return NextResponse.json({ error: 'Selection index unavailable' }, { status: 502 }) }
}
