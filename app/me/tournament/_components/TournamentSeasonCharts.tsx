...
export function TournamentSeasonCharts({
	...
	const formatHover = (point: TournamentPathPoint) => {
		if (mode === 'tournamentRank')
			return point.tournamentRank == null
				? '—'
				: t('formRankChip', { rank: point.tournamentRank })
		if (mode === 'gapToLeader') {
			const you =
				point.overallPoints == null ? '—' : format.number(point.overallPoints)
			const leader =
				point.leaderOverallPoints == null
					? '—'
					: format.number(point.leaderOverallPoints)
			const gap =
				point.gapToLeader == null
					? '—'
					: point.gapToLeader === 0
						? t('leading')
						: t('pointsBehindCount', {
								points: format.number(point.gapToLeader)
							})
			return t('chartHoverDual', {
				you,
				other: leader,
				otherLabel: t('chartSeriesLeader'),
				delta: gap
			})
		}
		const you =
		...
		return t('chartHoverDual', {
			you,
			other: average,
			otherLabel: t('chartSeriesAverage'),
			delta: difference
		})
	}
	...
	const updateHover = (point: { x?: string | number | null } | null) => {
		...
		setHover(
			Number.isFinite(gameweek)
				? (points.find(item => item.gameweek === gameweek) ?? null)
				: null
		)
	}
	...
	return (
		<StatsSectionCard
			...
			title={t('seasonPath')}
			description={
				latest?.tournamentRank != null
					? t('seasonPathHint', { rank: latest.tournamentRank })
					: undefined
			}
		>
			{state === 'UNAVAILABLE' ? (
				...
				<div className="space-y-4">
					<div>
						<p className="mb-2 text-caption font-medium text-muted-foreground">
							{t('formRecent', { count: form.length })}
						</p>
					...
					<div className="space-y-2 border-t border-border/60 pt-3">
						...
						{mode !== 'tournamentRank' ? (
							<ul className="flex flex-wrap gap-3 text-caption text-muted-foreground">
								<li className="inline-flex items-center gap-1.5">
									<span
										className="inline-block h-0.5 w-4 rounded-full bg-primary-ink"
										aria-hidden="true"
									/>
									{t('chartSeriesYou')}
								</li>
								<li className="inline-flex items-center gap-1.5">
									<span
										className="inline-block h-0.5 w-4 rounded-full bg-muted-foreground opacity-70"
										aria-hidden="true"
									/>
									{mode === 'gapToLeader'
										? t('chartSeriesLeader')
										: t('chartSeriesAverage')}
								</li>
							</ul>
						) : null}
						...
						{hover ? (
							...
							<p className="text-caption text-muted-foreground">
								{mode === 'tournamentRank'
									? t('chartTournamentRankHint')
									: mode === 'gapToLeader'
										? t('chartGapToLeaderHint')
										: t('chartPointsVsAverageHint')}
							</p>
						)}
					</div>
					{loading ? (
