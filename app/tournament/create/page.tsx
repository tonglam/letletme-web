...
const formSchema = z.object({
	tournamentName: z
		...
		.max(80, 'Tournament name must be at most 80 characters'),
	...
	leagueUrl: z
		...
		.refine(val => val === '' || fplUrlRegex.test(val), {
			message:
				'Please enter a valid Fantasy Premier League URL (e.g., https://fantasy.premierleague.com/leagues/12345/standings)'
		})
		.optional()
		.or(z.literal('')),
	participantCount: z
...
export default function CreateTournament() {
	...
	const [setupStatus, setSetupStatus] = useState<
		'pending' | 'processing' | 'ready' | 'failed' | null
	>(null)
	const [setupError, setSetupError] = useState<string | null>(null)
	const [createdParticipantCount, setCreatedParticipantCount] = useState<
		number | null
	>(null)
	...
	const computedGroupTeamNum =
		groupFormat === 'points'
			? Math.ceil(totalEntries / groupCount)
			: totalEntries
	...
	const computedKnockoutEventNum =
		computedKnockoutTeamNum >= 2
			? Math.ceil(Math.log2(computedKnockoutTeamNum))
			: 0
	const computedKnockoutRounds =
		knockoutFormat === 'double'
			? computedKnockoutEventNum * 2
			: computedKnockoutEventNum
	const computedKnockoutStartGwNumber =
		groupEndNumber > 0 ? groupEndNumber + 1 : 0
	...
	useEffect(() => {
		...
		const intervalId = window.setInterval(async () => {
			try {
				const response = await fetch(
					`/api/tournaments/setup-status?id=${createdTournamentId}`,
					{
						cache: 'no-store'
					}
				)
				...
				if (result.setupStatus === 'ready') {
					...
					setSubmitError(
						result.setupError || 'Tournament setup failed after creation.'
					)
					setSubmitSuccess(null)
				}
			} catch {
				if (!cancelled) {
					setSetupError(
						'Tournament was created, but setup status could not be refreshed.'
					)
				}
			}
		}, 3000)
			...
			try {
				...
				if (!hasCorrectDomain) {
					setUrlValidationError(
						`Invalid domain: '${url.hostname}'. Only 'fantasy.premierleague.com' is allowed.`
					)
					return
				}
				...
				if (fplUrlRegex.test(leagueUrl)) {
					...
					setUrlValidationError(
						'Invalid URL format. Expected format: https://fantasy.premierleague.com/leagues/12345/standings'
					)
				}
			} catch (e) {
				setIsDomainValid(false)
				setUrlValidationError(
					'Invalid URL. Please enter a complete URL including https://'
				)
			}
		} else {
			setUrlValidationError(null)
	...
	// TanStack Table intentionally exposes imperative functions; React Compiler
	// already skips this hook rather than memoizing stale table state.
	...
	const fetchParticipants = async () => {
		...
		try {
			...
			const fetchedParticipants: Participant[] = (
				result.participants || []
			).map((participant: ParticipantApiItem) => ({
				...participant,
	...
	const onSubmit = async (data: FormData) => {
		...
		if (!showTable) {
			if (!isValidUrl()) {
				setUrlValidationError(
					'Please enter a valid Fantasy Premier League URL and fetch participants before submitting'
				)
				return
			}

			setSubmitError(
				'Please fetch participants from the league URL before creating the tournament.'
			)
			return
		}

		if (
			data.participantSource === 'custom' &&
		...
		if (!groupStepReady) {
			setSubmitError(
				'Please complete the group phase settings before creating the tournament.'
			)
			return
		}
		...
		if (!knockoutStepReady) {
			setSubmitError(
				'Please complete the knockout phase settings before creating the tournament.'
			)
			return
		}
		...
		try {
			...
			const participantCount =
				result.tournament?.participantCount ?? participantIds.length
			const nextSetupStatus =
				result.setupStatus === 'processing' ||
				result.setupStatus === 'ready' ||
			...
			<div className="container max-w-4xl mx-auto px-4 py-8">
				...
				<form onSubmit={handleSubmit(onSubmit)}>
								...
								<div className="flex items-center gap-2">
									...
									<TooltipProvider>
										<Tooltip>
											...
											<TooltipContent>
												<p className="w-80">
													Enter a unique name for your tournament
												</p>
											</TooltipContent>
										</Tooltip>
									</TooltipProvider>
						...
						<div className="space-y-6">
							<div className="grid gap-3">
								...
								<p className="text-sm text-muted-foreground">
									Official includes all entries from the league URL; Custom lets
									you pick a subset after fetching.
								</p>
							</div>
							...
							<div className="space-y-3">
								<div className="flex items-center gap-2">
									<Label htmlFor="league-url">
										Official League URL <span className="text-red-500">*</span>
									</Label>
									<TooltipProvider>
										<Tooltip>
											<TooltipTrigger asChild>
											...
											<TooltipContent>
												<p className="w-80">
													Enter the full URL of your Fantasy Premier League
													league
												</p>
											</TooltipContent>
										</Tooltip>
									...
									<div className="flex-1">
										...
										{(errors.leagueUrl || urlValidationError) && (
											<p className="text-sm text-red-500 mt-1">
												{errors.leagueUrl?.message || urlValidationError}
											</p>
										)}
										<p className="text-xs text-muted-foreground mt-1">
											Format:
											https://fantasy.premierleague.com/leagues/12345/standings
										</p>
									</div>
									<Button
										type="button"
								...
								{!isDomainValid && leagueUrl && (
									<Alert className="bg-red-50 border-red-200 mt-3">
										...
										<AlertDescription className="text-red-800">
											<strong>Domain Error:</strong> Only URLs from
											fantasy.premierleague.com are allowed.
										</AlertDescription>
									</Alert>
								)}
								...
								{!showTable && isValidUrl() && (
									<Alert className="bg-blue-50 border-blue-200 mt-3">
										...
										<AlertDescription className="text-blue-800">
											The URL format is valid. Click &quot;Fetch&quot; to load
											participants from this league.
										</AlertDescription>
									</Alert>
								)}
										...
										<div className="mb-4 flex justify-end">
											<TooltipProvider>
												<Tooltip>
													...
													<TooltipContent>
														<p className="w-72">
															Fills the preset mode: Official data, Standard
															type, Points Race, GW1 to GW38, Group Num 1, and
															No Knockout.
														</p>
													</TooltipContent>
												</Tooltip>
						...
						<>
							<Card className="p-6 mb-8">
								...
								<div className="space-y-6">
									<div className="grid gap-3">
										<Label>
											Group Mode <span className="text-red-500">*</span>
										</Label>
										<Controller
											name="groupFormat"
											control={control}
									...
									<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
										<div className="space-y-3">
											<Label htmlFor="start-gameweek">
												Start Gameweek <span className="text-red-500">*</span>
											</Label>
											<Controller
												name="startGameweek"
												control={control}
										...
										<div className="space-y-3">
											<Label htmlFor="end-gameweek">
												End Gameweek <span className="text-red-500">*</span>
											</Label>
											<Controller
												name="endGameweek"
												control={control}
									...
									{groupFormat === 'points' &&
										...
										groupCount * groupQualifyCount > totalEntries && (
											<Alert className="bg-red-50 border-red-200">
												...
												<AlertDescription className="text-red-800">
													Group qualify total cannot exceed the total selected
													entries.
												</AlertDescription>
											</Alert>
										)}
							...
							{groupStepReady && (
								<Card className="p-6 mb-8">
									...
									<div className="space-y-6">
										<div className="grid gap-3">
											<Label>
												Knockout Mode <span className="text-red-500">*</span>
											</Label>
											<Controller
												name="knockoutFormat"
												control={control}
										...
										{knockoutFormat !== 'none' &&
											computedKnockoutEndGwNumber > 38 && (
												<Alert className="bg-red-50 border-red-200">
													<AlertCircle className="h-4 w-4 text-red-600" />
													<AlertDescription className="text-red-800">
														Knockout phase exceeds GW38. Reduce qualifying teams
														or use no knockout.
													</AlertDescription>
												</Alert>
											)}
					...
					{(submitError || submitSuccess || setupError) && (
						<Alert
							className={`mb-6 ${submitError ? 'bg-red-50 border-red-200' : 'bg-emerald-50 border-emerald-200'}`}
						>
							{submitError ? (
								<AlertCircle className="h-4 w-4 text-red-600" />
							) : (
							...
							<AlertDescription
								className={submitError ? 'text-red-800' : 'text-emerald-800'}
							>
								{submitError || submitSuccess || setupError}
							</AlertDescription>
						</Alert>
