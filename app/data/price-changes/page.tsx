...
const generateMockPriceChanges = (date: string): { rises: PriceChange[], falls: PriceChange[] } => {
  ...
  const falls = mockPlayers.slice(6, 11).map((player, index) => {
    const priceFall = (0.1 + (index * 0.05) + ((dateSeed + 500) % 100) / 1000) % 0.3;
    return {
      player,
...
const generatePlayerPriceHistory = (player: PlayerOption | null): PriceChange[] => {
  if (!player) return [];
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const previousPrice = isRise ? currentPrice - priceChange : currentPrice + priceChange;
...
export default function PriceChangesPage() {
  ...
  const [priceChanges, setPriceChanges] = useState<{ rises: PriceChange[], falls: PriceChange[] }>({ rises: [], falls: [] });
      ...
      <div className="container max-w-4xl mx-auto px-4 py-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="mb-8">
          <TabsList className="grid grid-cols-2 mb-4">
            ...
            <TabsTrigger value="player" disabled={!selectedPlayer}>Player Price History</TabsTrigger>
          </TabsList>
          
          <TabsContent value="daily">
            <Card className="p-6">
              <h2 className="text-2xl font-bold mb-2">Price Changes for {format(parseISO(selectedDate), "d MMMM yyyy")}</h2>
              <p className="text-muted-foreground mb-6">The latest price movements in the Fantasy Premier League market</p>
            ...
            {selectedPlayer && (
              <Card className="p-6">
                ...
                <p className="text-muted-foreground mb-6">
                  {selectedPlayer.position} | {selectedPlayer.team} | Current Price: £{selectedPlayer.price.toFixed(1)}m
                </p>
        ...
        <div className="bg-muted/30 p-4 rounded-lg text-sm text-muted-foreground">
          <p>Price changes typically occur at approximately 2:30 AM (UK time) each day during the FPL season. 
          The exact algorithm used by FPL is not public, but price changes are driven by transfer activity.</p>
        </div>
      </div>
    </RootLayout>
  );
