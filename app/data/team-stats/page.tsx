...
import { Team, teamFullNames } from '@/types/common'
...
const generateTeamStats = (team: Team, gameweek: number): TeamStats => {
	...
	let performanceModifier = topTier.includes(team)
		? 0.8
		: midTier.includes(team)
		? 0.5
		: 0.2
	...
	const goalsScored = Math.floor(
		wins * goalsPerWin + draws * goalsPerDraw + losses * goalsPerLoss
	)
	const goalsConceded = Math.floor(
		wins * concededPerWin + draws * concededPerDraw + losses * concededPerLoss
	)
	...
	const possession = Math.floor(
		40 + performanceModifier * 20 + (random(10) - 5)
	)
	const shotsBase = 10 + performanceModifier * 10
	const shots = Math.floor(shotsBase + (random(6) - 3))
	const shotsAccuracy = Math.floor(
		30 + performanceModifier * 30 + (random(10) - 5)
	)
	...
	const mostCaptained = {
		name: topScorer.name,
		captaincy: parseFloat(
			(5 + performanceModifier * 20 + random(10)).toFixed(1)
		)
	}

	const mostSelected = {
		name:
			players.find(p => p.form === Math.max(...players.map(p => p.form)))
				?.name || topScorer.name,
		selection: parseFloat(
			(20 + performanceModifier * 40 + random(20)).toFixed(1)
		)
	}
...
const generateTeamPlayers = (team: Team, performanceModifier: number) => {
	...
	const random = (max: number, min: number = 0) =>
		min + (((teamSeed % 100) + Math.random() * 100) % (max - min + 1))
...
const generateFixtures = (
	...
	for (let i = 0; i < 5; i++) {
		...
		const opponent = opponentOptions[
			Math.floor(Math.random() * opponentOptions.length)
		] as Team
									...
									>
										<div className="flex items-center gap-2">
											<div className="relative w-5 h-5">
												<Image
													src={`/images/team-logos/${team.toUpperCase()}.png`}
							...
							<div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
								<div className="flex flex-col items-center">
									<div className="relative w-28 h-28 mb-2">
										<Image
											src={`/images/team-logos/${teamStats.teamShort.toUpperCase()}.png`}
											alt={`${teamStats.team} logo`}
											fill
											className="object-contain"
										/>
									</div>
								...
								<Card className="p-6 mb-6">
									...
									<div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
										...
										<div className="bg-accent/30 rounded-lg p-3 text-center">
											...
											<p className="font-bold">
												{teamStats.shots} ({teamStats.shotsOnTarget})
											</p>
										</div>
										<div className="bg-accent/30 rounded-lg p-3 text-center">
											<p className="text-xs text-muted-foreground mb-1">
								...
								<Card className="p-6">
									...
									<div className="grid grid-cols-2 gap-4">
										...
										<div className="bg-accent/30 rounded-lg p-3">
											...
											<p className="font-bold">
												{teamStats.topAssister.name} (
												{teamStats.topAssister.assists} assists)
											</p>
										</div>
									</div>
								</Card>
								...
								<Card className="p-6">
									...
									<div className="grid grid-cols-2 gap-4 mb-6">
										<div className="bg-accent/30 rounded-lg p-3">
											...
											<p className="font-bold">
												{teamStats.mostCaptained.name}
											</p>
											<p className="text-xs text-muted-foreground mt-1">
												Captained by {teamStats.mostCaptained.captaincy}% of
												managers
											</p>
										</div>
										<div className="bg-accent/30 rounded-lg p-3">
											...
											<p className="text-xs text-muted-foreground mt-1">
												Selected by {teamStats.mostSelected.selection}% of
												managers
											</p>
										</div>
									</div>
