	...
	return (
		...
		>
			<div className="flex items-center justify-between gap-2">
				<span className={cn('eyebrow', 'text-foreground')}>{label}</span>
				<span
					className={cn(
...
export default function FixturesClient({
	...
	const softestNext = useMemo(() => {
		const withNext = model.teams.filter(
			row => row.nextFdr != null && row.run[0]
		)
	...
	const jumpToTeam = useCallback(
		(teamId: number | undefined, nextSort: 'easiest' | 'hardest') => {
					...
					>
						...
						{softestNext?.run[0] ? (
							<GlanceNextCard
								...
								tone="easy"
								onClick={() => jumpToTeam(softestNext.teamId, 'easiest')}
								ariaLabel={t('glanceJumpToTeam', {
						...
						{hardestNext?.run[0] ? (
							<GlanceNextCard
								...
								tone="hard"
								onClick={() => jumpToTeam(hardestNext.teamId, 'hardest')}
								ariaLabel={t('glanceJumpToTeam', {
