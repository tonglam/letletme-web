...
import {
	TOURNAMENT_STATS_MOCK_ENTRY_ID,
	TOURNAMENT_STATS_MOCK_EVENT_ID,
	TOURNAMENT_STATS_MOCK_TOURNAMENT_ID,
...
export default async function TournamentStatsPage({
	...
	if (TOURNAMENT_STATS_UI_MOCK_ENABLED) {
		const tournament = getTournamentStatsUiMockTournament(
			TOURNAMENT_STATS_MOCK_ENTRY_ID,
		)
		const rows = getTournamentStatsUiMockEventResults(
			TOURNAMENT_STATS_MOCK_EVENT_ID,
			TOURNAMENT_STATS_MOCK_ENTRY_ID,
		)
		return (
			<TournamentStatsClient
				entryId={TOURNAMENT_STATS_MOCK_ENTRY_ID}
				initialCurrentGameweek={TOURNAMENT_STATS_MOCK_EVENT_ID}
				initialTournaments={[tournament]}
				initialSelectedTournamentId={String(TOURNAMENT_STATS_MOCK_TOURNAMENT_ID)}
				initialDataGameweek={TOURNAMENT_STATS_MOCK_EVENT_ID}
			...
			if (
				...
				if (currentRows.length === 0) {
					for (let offset = 1; offset <= 4; offset += 1) {
						...
						const fallbackRows = await fetchTournamentResults(
							tournamentId,
							fallbackGw,
						)
