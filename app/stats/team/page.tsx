...
const fetchCurrentEventIdSafely = async (): Promise<number | null> => {
  try {
    const endpoint = getGraphQLEndpointForClient();
    const response = await fetch(endpoint, {
      method: "POST",
    ...
    return payload.data?.current[0]?.id ?? null;
  } catch {
    return null;
  }
};
...
let currentEventIdCache: number | null | undefined = undefined;
let currentEventIdInFlight: Promise<number | null> | null = null;

const getCurrentEventIdCached = async (): Promise<number | null> => {
  if (currentEventIdCache !== undefined) {
    return currentEventIdCache;
  }
  if (!currentEventIdInFlight) {
    currentEventIdInFlight = fetchCurrentEventIdSafely()
      .then((eventId) => {
        currentEventIdCache = eventId;
        return eventId;
      })
      .finally(() => {
        currentEventIdInFlight = null;
      });
  }
  return currentEventIdInFlight;
};

const getEntryHistoryCached = async (entryId: number): Promise<EntryHistoryResponse["entryHistory"]> => {
  if (entryHistoryCache) {
    return entryHistoryCache;
  }
  if (!entryHistoryInFlight) {
    entryHistoryInFlight = executeQuery<EntryHistoryResponse>(
      GET_ENTRY_HISTORY,
      { entryId },
      { cache: "force-cache" },
    )
      .then((response) => {
        entryHistoryCache = response.entryHistory;
        return response.entryHistory;
      })
      .finally(() => {
        entryHistoryInFlight = null;
      });
  }
...
const getEntryEventResultCached = async (entryId: number, eventId: number): Promise<EntryEventResult | null> => {
  if (entryEventCache.has(eventId)) {
    return entryEventCache.get(eventId) ?? null;
  }
  ...
  const request = executeQuery<EntryEventResultResponse>(
    ...
    .finally(() => {
      entryEventInFlightCache.delete(eventId);
    });
...
const getTransferHistoryCached = async (entryId: number): Promise<EntryGameweekTransfers[]> => {
  ...
  if (!transferHistoryInFlight) {
    transferHistoryInFlight = fetchTransferHistorySafely(entryId)
      .then((response) => {
        transferHistoryCache = response;
        return response;
      })
      .finally(() => {
        transferHistoryInFlight = null;
      });
  }
...
const isTeamStatsTab = (value: string): value is TeamStatsTab => {
  return value === "squad" || value === "transfer" || value === "history" || value === "chips";
};
...
export default function TeamStatsPage() {
  ...
  useEffect(() => {
    const bootstrap = async () => {
      ...
      const currentEventId = await getCurrentEventIdCached();
      if (currentEventId !== null) {
        setCurrentGameweek(currentEventId);
        setSelectedGameweek(currentEventId);
      }
                          ...
                          return (
                            ...
                            >
                              ...
                              <TableCell className="text-center font-bold">
                                <span
                                  className={pick.totalPoints > 0 ? "text-emerald-600 dark:text-emerald-400" : ""}
                                >
                                  {pick.totalPoints}
                                </span>
                              </TableCell>
