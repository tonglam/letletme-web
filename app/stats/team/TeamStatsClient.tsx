...
const fetchTransferHistorySafely = async (
	entryId: number
	...
	try {
		...
		const payload = (await response.json()) as {
			data?: EntryTransferHistoryResponse
			errors?: unknown
		}
...
const getEntryHistoryCached = async (
	entryId: number
): Promise<EntryHistoryResponse['entryHistory']> => {
	...
	const request = executeQuery<EntryHistoryResponse>(
		...
		.then(response => {
			entryHistoryCache.set(entryId, response.entryHistory)
...
const getEntryEventResultCached = async (
	entryId: number,
	eventId: number
): Promise<EntryEventResult | null> => {
	const cacheKey = entryEventCacheKey(entryId, eventId)
	if (entryEventCache.has(cacheKey))
		return entryEventCache.get(cacheKey) ?? null
...
const getTransferHistoryCached = async (
	entryId: number
): Promise<EntryGameweekTransfers[]> => {
	if (transferHistoryCache.has(entryId))
		return transferHistoryCache.get(entryId)!
	const inflight = transferHistoryInFlight.get(entryId)
	if (inflight) return inflight
	const request = fetchTransferHistorySafely(entryId)
		.then(response => {
			transferHistoryCache.set(entryId, response)
			return response
		})
		.finally(() => {
...
export default function TeamStatsClient({
	...
	const entryId = sessionData?.user?.fplEntryVerifiedAt
		...
		const loadTeamStats = async () => {
			try {
				...
				const historyRequired =
					...
					activeTab === 'chips' ||
					activeTab === 'transfer'
				const entryHistory = historyRequired
					? await getEntryHistoryCached(entryId)
					: null
				...
				const entryTransferHistory =
					activeTab === 'transfer'
						? await getTransferHistoryCached(entryId)
						: null
						...
						<Card className="p-6 mb-6">
							...
							<div className="grid grid-cols-3 gap-4">
								<div className="bg-accent/30 p-4 rounded-lg text-center">
									...
									<p className="text-2xl font-bold">
										{teamStats.teamValue === null
											? '-'
											: `£${(teamStats.teamValue / 10).toFixed(1)}m`}
									</p>
								</div>
								<div className="bg-accent/30 p-4 rounded-lg text-center">
								...
								<div className="bg-accent/30 p-4 rounded-lg text-center">
									...
									<p className="text-2xl font-bold">
										{teamStats.totalTransfers === null
											? '-'
											: teamStats.totalTransfers}
									</p>
								</div>
							</div>
							<div className="grid grid-cols-2 gap-4 mt-4">
								...
								<div className="bg-accent/30 p-4 rounded-lg text-center">
									...
									<p className="text-2xl font-bold">
										£{formatCompact(teamStats.overallRank)}
									</p>
								</div>
							</div>
						</Card>
						...
						<Card className="p-6 mb-6">
							<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
								...
								<div className="bg-accent/30 p-4 rounded-lg text-center">
									...
									<p className="text-2xl font-bold">
										{teamStats.eventTransfersCost > 0
											? `-${teamStats.eventTransfersCost}`
											: '0'}
									</p>
								</div>
								<div className="bg-accent/30 p-4 rounded-lg text-center">
							...
							<div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
								...
								<div className="bg-accent/30 p-4 rounded-lg text-center">
									...
									<p className="text-base font-semibold">
										{teamStats.eventPlayedCaptainName} (
										{teamStats.eventCaptainPoints})
									</p>
								</div>
							</div>
						</Card>
																		...
																		row.moves.map((move, index) => (
																			...
																			>
																				<p className="text-sm font-medium leading-5">
																					...
																					{move.inName} ({move.inTeam})
																				</p>
																				<p className="text-xs text-muted-foreground mt-1.5">
																					Sold:{' '}
																					{formatPlayerValue(move.outCost)} |
																					Bought:{' '}
																					{formatPlayerValue(move.inCost)}
																				</p>
																			</div>
																		))
																	)}
									...
									<StatsTable
										...
										columns={[
											...
											},
											{
												...
												format: value =>
													formatMoney(value == null ? null : Number(value))
											},
											{
												key: 'bank',
												label: 'Bank',
								...
								<Card className="p-6">
									...
									<div className="grid grid-cols-2 gap-4 mb-6">
										...
										<div className="bg-accent/30 p-4 rounded-lg text-center">
											...
											<p className="text-2xl font-bold">
												{teamStats.chipUsageRows.length}
											</p>
										</div>
									</div>
